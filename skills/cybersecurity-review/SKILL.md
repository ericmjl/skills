...
description: >
  Perform comprehensive cybersecurity code review across 7 security dimensions:
  injection prevention, authentication/authorization, secrets management,
  supply chain security, cryptography, secure configuration/API security,
  and error handling/logging/resource safety. Use when reviewing newly written code,
...
Perform structured security code review across 7 dimensions, adapting depth based on review mode.
...
|------|---------|-------|-------|
| **New Code** | Reviewing code just written or a new feature | Changed files only | Deep on all 7 dimensions |
| **Existing Repo** | Auditing an established codebase | Full repository scan | Prioritize high-severity, sample for depth |
| **Open Source Eval** | Evaluating a dependency or OSS project | Full project + community signals | Supply chain focus + all 7 dimensions |
...
4. **Dependency & Supply Chain Security** -- See [references/supply-chain-security.md](references/supply-chain-security.md)
   - Vulnerable dependencies, typosquatting, dependency confusion, lockfile integrity
...
6. **Secure Configuration & API Security** -- See [references/config-and-api-security.md](references/config-and-api-security.md)
   - Debug mode, permissive CORS, missing security headers, BOLA, mass assignment, rate limiting

7. **Error Handling, Logging & Resource Safety** -- See [references/error-logging-resources.md](references/error-logging-resources.md)
   - Stack trace exposure, sensitive data in logs, log injection, ReDoS, buffer overflows, TOCTOU
...
### New Code Review
Focus on all 7 dimensions with equal weight. Check that new code does not introduce patterns flagged in the reference files. Verify that new dependencies are pinned and scanned.

### Existing Repo Audit
Start with a high-level scan using grep patterns from each reference file. Prioritize Critical/High findings. Sample representative files for deeper review. Check for missing security controls (CSP headers, rate limiting, auth middleware).

### Open Source Evaluation
Begin with supply chain signals: maintenance activity, contributor count, security policy presence, OpenSSF Scorecard if available. Then review code for all 7 dimensions. Flag any use of dangerous patterns (eval, pickle, shell=True) as higher risk since the code runs in consumer environments.

### Pull Request Review
Focus on the diff. For each changed file, assess which dimensions are relevant based on the code's function. Flag new dangerous patterns. Verify that security-sensitive changes have tests. Check that secrets were not accidentally committed.
