---
name: skill-creator
description: Guide for authoring Agent Skills with strong YAML `description` triggers, progressive disclosure, and bundled resources. Use when creating or updating a skill, running init_skill.py or package_skill.py, or improving a bland skill description so agents load the skill on the right user tasks.
license: MIT
---
...
**Deep dive:** [references/skill-description-triggers.md](references/skill-description-triggers.md) (checklist, anti-patterns, links to [agentskills.io](https://agentskills.io/skill-creation/optimizing-descriptions)).
...
- `name`: The skill name
- `description`: This is the **only** triggering signal agents have before loading the skill.
  - **Structure:** Capabilities (what it does) + imperative **Use when …** clause(s) + concrete anchors (user phrasings, paths, tools, hosts, file types).
  - Put **all** “when to use” material here. The body loads only after the skill triggers, so a body-only “When to use this skill” section does **not** help discovery.
  - **Test:** Read only `name` + `description`. Would you invoke this skill for each of the 3–5 example requests from Step 1?
  - **Length:** Stay under 1024 characters (spec). This repository’s validator also requires at least ~80 characters so single-sentence generic blurbs fail early.
