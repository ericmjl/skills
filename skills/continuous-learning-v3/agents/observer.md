---
description: Background agent that analyzes session observations to detect patterns and create instincts. Agent-agnostic design for any AI coding assistant.
---

# Observer Agent

A background agent that analyzes observations from AI coding agent sessions to detect patterns and create instincts.
...
Reads observations from project-scoped observations file:
- Project: `$AGENT_LEARNING_HOME/projects/<project-hash>/observations.jsonl`
- Global fallback: `$AGENT_LEARNING_HOME/observations.jsonl`

```jsonl
{"timestamp":"2025-01-22T10:30:00Z","event":"tool_start","session":"abc123","tool":"Edit","input":"...","project_id":"a1b2c3d4e5f6","project_name":"my-react-app","agent":"claude-code"}
{"timestamp":"2025-01-22T10:30:01Z","event":"tool_complete","session":"abc123","tool":"Edit","output":"...","project_id":"a1b2c3d4e5f6","project_name":"my-react-app","agent":"claude-code"}
{"timestamp":"2025-01-22T10:30:05Z","event":"tool_start","session":"abc123","tool":"Bash","input":"npm test","project_id":"a1b2c3d4e5f6","project_name":"my-react-app","agent":"claude-code"}
{"timestamp":"2025-01-22T10:30:10Z","event":"tool_complete","session":"abc123","tool":"Bash","output":"All tests pass","project_id":"a1b2c3d4e5f6","project_name":"my-react-app","agent":"claude-code"}
```
...
Creates/updates instincts in project-scoped instincts directory:
- Project: `$AGENT_LEARNING_HOME/projects/<project-hash>/instincts/personal/`
- Global: `$AGENT_LEARNING_HOME/instincts/personal/` (for universal patterns)
...
**When in doubt, default to `scope: project`** — safer to be project-specific and promote later.
...
Track which agent learned each pattern via the `agent` field:
- `"claude-code"` — Claude Code
- `"cursor"` — Cursor IDE
- `"gemini"` — Gemini CLI
- `"opencode"` — OpenCode
- `"any"` — Universal (learned from multiple agents)

When an instinct is observed across multiple agents with high confidence, set `agent: "any"`.

## Instinct Promotion (Project → Global)

Promote when:
1. Same pattern exists in **2+ different projects**
2. Each instance has confidence **>= 0.8**
3. The domain is global-friendly (security, best-practices, workflow)
...
6. **Default to Project Scope**: Unless clearly universal, make it project-scoped
7. **Include Project Context**: Always set `project_id` and `project_name` for project-scoped instincts
8. **Track Agent**: Always set the `agent` field to identify the source
