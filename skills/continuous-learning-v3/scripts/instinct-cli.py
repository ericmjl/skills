...
# ///
"""
Instinct CLI - Manage instincts for Continuous Learning v3

Agent-agnostic version that works with any AI coding agent.

Usage:
    uv run instinct-cli.py status
    uv run instinct-cli.py evolve
    uv run instinct-cli.py --help
...
Commands:
  ...
  migrate  - Migrate data from v2.x location
"""
...
HOMUNCULUS_DIR_V2 = Path.home() / ".claude" / "homunculus"

BASE_DIR = Path(
    os.environ.get("AGENT_LEARNING_HOME", str(Path.home() / ".agent-learning"))
)
PROJECTS_DIR = BASE_DIR / "projects"
REGISTRY_FILE = BASE_DIR / "projects.json"

GLOBAL_INSTINCTS_DIR = BASE_DIR / "instincts"
GLOBAL_PERSONAL_DIR = GLOBAL_INSTINCTS_DIR / "personal"
GLOBAL_INHERITED_DIR = GLOBAL_INSTINCTS_DIR / "inherited"
GLOBAL_EVOLVED_DIR = BASE_DIR / "evolved"
GLOBAL_OBSERVATIONS_FILE = BASE_DIR / "observations.jsonl"
...
def detect_project() -> dict:
    ...
    env_dir = os.environ.get("AGENT_PROJECT_DIR") or os.environ.get(
        "CLAUDE_PROJECT_DIR"
    )
...
def _print_instincts_by_domain(instincts: list[dict]) -> None:
    ...
    for domain in sorted(by_domain.keys()):
        ...
        for inst in sorted(domain_instincts, key=lambda x: -x.get("confidence", 0.5)):
            ...
            agent_tag = f"({inst.get('agent', 'any')})"

            print(
                f"    {conf_bar} {int(conf * 100):3d}%  {inst.get('id', 'unnamed')} {scope_tag} {agent_tag}"
            )
...
def cmd_import(args) -> int:
    ...
    for inst in all_to_write:
        ...
        if inst.get("agent"):
            output_content += f"agent: {inst.get('agent')}\n"
        output_content += "---\n\n"
...
def _promote_specific(project: dict, instinct_id: str, force: bool) -> int:
    ...
    if target.get("agent"):
        output_content += f"agent: {target.get('agent')}\n"
    output_content += "---\n\n"
...
def cmd_projects(args) -> int:
    ...
    if not registry:
        ...
        print("Projects are auto-detected when you use an AI agent in a git repo.")
        return 0
...
def cmd_migrate(args) -> int:
    v2_dir = Path(args.from_dir) if args.from_dir else HOMUNCULUS_DIR_V2

    if not v2_dir.exists():
        print(f"v2.x directory not found: {v2_dir}")
        return 1

    print(f"Migrating from v2.x: {v2_dir}")
    print(f"To v3.x: {BASE_DIR}")
    print()

    migrated = 0

    for instinct_type in ["personal", "inherited"]:
        src_dir = v2_dir / "instincts" / instinct_type
        dst_dir = GLOBAL_INSTINCTS_DIR / instinct_type
    ...
    v2_projects = v2_dir / "projects"
    if v2_projects.exists():
        for proj_dir in v2_projects.iterdir():
            if proj_dir.is_dir():
                dst_proj = PROJECTS_DIR / proj_dir.name
                if not dst_proj.exists():
                    import shutil

                    shutil.copytree(proj_dir, dst_proj)
                    print(f"  Migrated project: {proj_dir.name}")

    v2_registry = v2_dir / "projects.json"
    if v2_registry.exists():
        try:
            with open(v2_registry) as f:
                v2_data = json.load(f)
            ...
            if not REGISTRY_FILE.exists():
                ...
                print(f"  Migrated registry: {len(v2_data)} projects")
        except Exception as e:
            print(f"  Warning: Could not migrate registry: {e}")

    print()
    print(f"Migration complete! Migrated {migrated} instinct files.")
    print()
    print("To remove v2.x data after verifying migration:")
    print(f"  rm -rf {v2_dir}")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Instinct CLI for Continuous Learning v3 (Agent-Agnostic)"
    )
    ...
    migrate_parser = subparsers.add_parser(
        "migrate", help="Migrate data from v2.x location"
    )
    migrate_parser.add_argument(
        "--from-dir", help="v2.x directory (default: ~/.claude/homunculus)"
    )
